        ...
        <div class="bg-white shadow-md" x-data="{ isOpen: false }">
            <nav class="container px-6 py-8 mx-auto md:flex md:justify-between md:items-center" x-data="{ open: false }" wire:id="oYMiPnZp4Nzbmr9iAsGp">
              ...
              <div :class="isOpen ? 'flex' : 'hidden'" class="flex-col mt-8 space-y-4 md:flex md:space-y-0 md:flex-row md:items-center md:space-x-10 md:mt-0 justify-center">
                ...
                <a  href="{{ Auth::check() ? route('dashboard.index') : route('welcome.index') }}"class="text-transparent bg-clip-text bg-gradient-to-r from-red-700 via-orange-400 to-red-800 hover:text-green-400" href="#">Home</a>
                @foreach ( $categories as $category )
                <a href="{{ route('categorydetails', ['id' => $category->id]) }}" class="text-transparent bg-clip-text bg-gradient-to-r from-red-800 via-yellow-400 to-red-800 hover:text-green-400">{{$category->name}}</a>
                @endforeach
                @can('admin')
